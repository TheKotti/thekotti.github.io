...
function createContainerObject() {
  ...
  for (var prop in generic)
    if (generic.hasOwnProperty(prop)) container[prop] = generic[prop];
  ...
  if (mode === "RANDOM") {
    var allMelee = [];
    randomMissionList.forEach(element => {
      allMelee = allMelee.concat(element.melee);
    });
    allMelee = Array.from(new Set(allMelee));
    container.melee = allMelee;
...
function createExtrasList(exit) {
  if (!document.getElementById("restrictions").checked) return [];
  ...
  if (Math.random() < 0.18) extras.push("Do not throw items as distractions.");

  if (Math.random() < 0.25)
    extras.push("Do not use firearms as distractions or to destroy objects.");

  if (Math.random() < 0.12) extras.push("Do not climb.");

  if (Math.random() < 0.05) extras.push("Do not crouch.");
...
function createTargetList(container) {
  ...
  } else if (mode == "ELUSIVE") targets = ["Elusive Target"];
  else {
...
function containerToResult(container) {
  ...
  result.entry =
    container.entry[Math.floor(Math.random() * container.entry.length)];
  result.exit =
    container.exit[Math.floor(Math.random() * container.exit.length)];
  return result;
}
